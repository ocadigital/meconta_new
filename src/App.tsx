...
import { 
  ...
  Settings, AlertTriangle, LogOut, Shield, Menu, X, LayoutDashboard, MessageSquare, Bell, FileText, Lock, Trash2, CheckCircle, Store, Tag, AlignLeft, DollarSign, Edit2, Ban, Eye, PanelLeft, Gift, Share2, Copy, Users
} from 'lucide-react';
import { Transaction, TransactionType, User, BankAccount, BankAccountType, CardBrand, PaymentMethod, CreditCard } from './types';
...
export default function App() {
  ...
  const handleDeleteAccount = async (accId: string) => {
      ...
      const hasTrans = transactions.some(t => t.accountId === accId);
      if (hasTrans) {
  ...
  const handleDeleteCard = async (cardId: string) => {
      const hasTrans = transactions.some(t => t.cardId === cardId);
      if (hasTrans) {
          showToast("Não é possível excluir: existem transações vinculadas a este cartão.", "error");
          return;
      }
  ...
  const handleEditAccount = (acc: BankAccount) => {
      setAccountForm({
          ...
          type: acc.type,
          color: acc.color,
          balance: (acc.balance || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
          brand: 'MASTERCARD',
          limit: '',
          closingDay: '',
          dueDay: ''
  ...
  const handleEditCard = (card: CreditCard) => {
      setAccountForm({
          ...
          balance: '',
          brand: card.brand,
          limit: (card.limit || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 }),
  ...
  const handleAccountBalanceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      const val = e.target.value.replace(/[^0-9,.]/g, '');
      setAccountForm({...accountForm, balance: val});
  };

  const calculateCurrentBalance = (acc: BankAccount) => {
      const start = acc.balance || 0;
      const moves = transactions.filter(t => t.accountId === acc.id && t.isPaid);
      const total = moves.reduce((sum, t) => {
  ...
  if (!currentUser) {
      if (showAuthScreen) return <AuthScreen onLogin={handleLogin} users={users} onRegister={storageService.register} initialView={authInitialView} />;
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans">
       ...
       <header className="bg-white shadow-sm sticky top-0 z-30 px-4 h-16 flex justify-between items-center">
           ...
           <div className="flex items-center gap-4">
               {/* Referral Button */}
               <button onClick={() => setShowReferralModal(true)} className="p-2 text-gray-500 hover:bg-green-50 hover:text-green-600 rounded-full transition-colors relative">
               ...
               {showReferralModal && (
                   <div className="absolute top-16 right-20 w-80 bg-white shadow-xl rounded-xl border border-gray-100 z-50 p-6 animate-in slide-in-from-top-2">
                       <div className="flex items-center gap-3 mb-4">
                           <div className="bg-green-100 p-2 rounded-full"><Gift className="w-6 h-6 text-green-600" /></div>
                           <div>
                               <h3 className="font-bold text-gray-900">Indique amigos!</h3>
                               <p className="text-xs text-gray-500">Compartilhe o MeConta</p>
                           </div>
                       </div>
                       ...
                       <div className="bg-gray-50 p-3 rounded-lg flex justify-between items-center mb-4 border border-gray-200">
                           <div className="flex items-center gap-2 text-sm text-gray-600 font-mono">
                               ...
                               <span>Código: <span className="font-bold text-black">{currentUser.id.slice(0, 8).toUpperCase()}</span></span>
                           </div>
                       </div>
                       ...
                       <div className="flex gap-2">
                           <button 
                                ...
                                className="flex-1 border border-gray-200 hover:bg-gray-50 text-gray-700 py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2"
                           >
                               <Copy className="w-4 h-4" /> Copiar link
                           </button>
                           <button 
                                onClick={() => {
                                    const text = `Organize suas finanças com o MeConta! Use meu link: ${window.location.origin}?ref=${currentUser.id}`;
                                    window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank');
                                }}
                                className="flex-1 bg-green-500 hover:bg-green-600 text-white py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2 shadow-sm"
                           >
                               <MessageCircle className="w-4 h-4" /> Enviar
                           </button>
                       </div>
                       <button onClick={() => setShowReferralModal(false)} className="absolute top-2 right-2 text-gray-400 hover:text-gray-600"><X className="w-4 h-4"/></button>
                   </div>
               )}
       ...
       <div className="max-w-7xl mx-auto p-4 flex gap-6">
           <aside className={`hidden md:block bg-white rounded-none border-r border-gray-100 h-[calc(100vh-64px)] fixed left-0 top-16 overflow-y-auto z-20 transition-all duration-300 ${sidebarCollapsed ? 'w-20' : 'w-64'}`}>
               <div className="p-4 space-y-6">
                   ...
                   <nav className="space-y-1">
                       ...
                       {currentUser.isAdmin && <button onClick={() => setView('admin')} className={`w-full text-left px-4 py-3 rounded-lg flex gap-3 font-medium transition-colors ${view === 'admin' ? 'text-blue-600 bg-blue-50' : 'text-gray-500 hover:bg-gray-50'} ${sidebarCollapsed ? 'justify-center px-2' : ''}`} title={sidebarCollapsed ? "Admin Panel" : ""}><Shield className="w-5 h-5 flex-shrink-0"/> {!sidebarCollapsed && <span>Admin Panel</span>}</button>}
           ...
           <main className={`flex-1 min-w-0 pt-6 transition-all duration-300 ${sidebarCollapsed ? 'md:ml-20' : 'md:ml-64'}`}>
               ...
               {view === 'admin' && currentUser.isAdmin && <AdminPanel />}
               ...
               {view === 'wallet' && (
                   <div className="space-y-4">
                       <div className="flex justify-between items-center mb-2">
                            ...
                            <button onClick={() => { setAccountForm({...accountForm, type: 'CHECKING', accountId: undefined, balance: ''}); setShowAccountModal(true); }} className="bg-green-700 text-white px-4 py-2 rounded-lg font-bold text-sm flex items-center gap-2"><Plus className="w-4 h-4"/> Nova Conta</button>
                       </div>
                           ...
                           {accounts.map(acc => (
                               <div key={acc.id} className={`${getAccountColorClass(acc.name, acc.type)} rounded-2xl p-6 w-80 shadow-lg relative flex flex-col justify-between min-h-[200px] group`}>
                                   <div>
                                       ...
                                       <p className="text-white/70 text-xs font-bold uppercase">{translateAccountType(acc.type)}</p>
                                       {/* Current Balance Display */}
                                       <p className="text-2xl font-bold mt-2">{formatCurrency(calculateCurrentBalance(acc))}</p>
                                   </div>
                                   ...
                                   <div className="mt-4">
                                        {acc.cards && acc.cards.map(card => (
                                            <div key={card.id} className="bg-white/20 rounded-lg p-2 mb-2 text-white text-sm font-bold flex justify-between group/card relative">
                                                <div className="flex flex-col">
                                                    <span>{card.name}</span>
                                                    <span className="text-[10px] opacity-75">Vence dia {card.dueDay || '?'}</span>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <span>Limit: {card.limit}</span>
                                                    <div className="hidden group-hover/card:flex gap-1 absolute right-2 bg-black/50 rounded p-1">
                                                        <button onClick={(e) => { e.stopPropagation(); handleEditCard(card); }}><Edit2 className="w-3 h-3 text-white"/></button>
                                                        <button onClick={(e) => { e.stopPropagation(); handleDeleteCard(card.id); }}><Trash2 className="w-3 h-3 text-red-400"/></button>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
               ...
               {view === 'categories' && (
                   <div className="space-y-6">
                       {/* Categories View Content - Unchanged */}
               ...
               {showAccountModal && (
                   <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                       <div className="bg-white rounded-2xl w-full max-w-sm p-6">
                           <h3 className="font-bold text-lg mb-4">{accountForm.type === 'CREDIT_CARD' ? 'Dados do Cartão' : 'Nova Conta'}</h3>
                           <form onSubmit={handleAccountSubmit} className="space-y-4">
                               ...
                               {accountForm.type !== 'CREDIT_CARD' && (
                                   <div>
                                       <label className="text-[10px] font-bold text-gray-500 uppercase mb-1 block">Saldo Inicial (R$)</label>
                                       <input type="text" placeholder="0,00" value={accountForm.balance} onChange={handleAccountBalanceChange} className="w-full p-3 border rounded-xl" />
                                   </div>
                               )}
                               ...
                               {accountForm.type === 'CREDIT_CARD' && (
                                   <div className="flex gap-3">
                                       <div>
                                           <label className="text-[10px] font-bold text-gray-500 uppercase mb-1 block">Fechamento</label>
                                           <input type="number" min="1" max="31" placeholder="Dia" value={accountForm.closingDay} onChange={e => setAccountForm({...accountForm, closingDay: e.target.value})} className="w-full p-3 border rounded-xl" />
                                       </div>
                                       <div>
                                           <label className="text-[10px] font-bold text-gray-500 uppercase mb-1 block">Vencimento</label>
                                           <input type="number" min="1" max="31" placeholder="Dia" value={accountForm.dueDay} onChange={e => setAccountForm({...accountForm, dueDay: e.target.value})} className="w-full p-3 border rounded-xl" />
                                       </div>
                                   </div>
                               )}
