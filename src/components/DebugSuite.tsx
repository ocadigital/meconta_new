...
export const DebugSuite: React.FC = () => {
    ...
    const runTests = async () => {
        ...
        try {
            ...
            if (Array.isArray(users)) update(0, 'success', `Conectado. ${users.length} usuários encontrados.`);
        ...
        try {
            ...
            update(1, 'success', "Schema verificado e mapeamento OK.");
        } catch (e: any) {
        ...
        try {
            ...
            if (created.length >= 12) {
                update(2, 'success', `Sucesso! Sistema gerou ${created.length} transações para um lançamento fixo.`);
            } else {
                // Se falhar, tenta limpar o que foi criado
                update(2, 'error', `Falha. Esperado 12, gerado ${created.length}. Verifique backend.`);
            }
        ...
        try {
            ...
            if (failures.length === 0) {
                 update(3, 'success', `Todos os menus verificados: ${checks.map(c => c.page).join(', ')}.`);
            } else {
