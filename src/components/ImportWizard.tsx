...
import { Upload, FileText, CreditCard, ArrowRight, Check, X, AlertTriangle, Wand2, Loader2, Calendar, DollarSign, Tag, CheckCircle } from 'lucide-react';
...
export const ImportWizard: React.FC<ImportWizardProps> = ({ accounts, existingTransactions, onClose, onImportFinished, categories }) => {
    ...
    const parseOFX = async (text: string): Promise<StagedTransaction[]> => {
        // Very basic regex-based OFX parser for client-side
    ...
    const processFile = async () => {
        ...
        try {
            ...
            if (importType === 'OFX') {
                ...
                // PDF - Convert to Base64 and send to AI
            ...
            const checked = parsed.map(t => {
                ...
                return { ...t, isDuplicate: dup, selected: !dup }; // Deselect duplicates by default
            });
        ...
        try {
            ...
            for (const t of toImport) {
                const finalTrans: Transaction = {
                    ...
                    userId,
                    memberId: userId, // Default to self
                    date: t.date,
                    description: t.description,
                    amount: t.amount,
                    type: t.type,
                    ...
                    store: '',
                    isFixed: false,
                    isPaid: true, // Imported usually means paid/posted
                    isPrivate: false
                };
