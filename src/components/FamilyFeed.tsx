...
export const FamilyFeed: React.FC = () => {
    ...
    const toggleReaction = async (transactionId: string, type: 'LIKE' | 'CLAP') => {
        setFeed(prev => prev.map(item => {
            if (item.id === transactionId) {
                ...
                const updatedReactions = { ...(item.reactions || {}), [type]: newCount };
                return { ...item, userReaction: hasReacted ? null : type, reactions: updatedReactions };
            }
            return item;
        }));
    ...
    if (error) return <div className="p-8 text-center text-red-400">Erro ao carregar feed: {error}</div>;
    ...
    return (
        <div className="space-y-6 max-w-3xl">
            {feed.map((item) => {
                ...
                // SAFEGUARD: Ensure userName exists to prevent crash
                const safeUserName = item.userName || 'Usuário';
                    ...
                    <div key={item.id} className={`bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border ${isGoal ? 'border-purple-200 dark:border-purple-900/30 ring-1 ring-purple-100 dark:ring-purple-900/20' : 'border-gray-100 dark:border-gray-700'} relative overflow-hidden group hover:shadow-md transition-all`}>
                        <div className="flex justify-between items-start mb-3">
                            <div className="flex items-center gap-3">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold shadow-sm ${item.avatarColor || 'bg-gray-400'}`}>{safeInitial}</div>
                                <div><p className="text-sm font-bold text-gray-900 dark:text-white leading-tight">{safeUserName}</p><p className="text-xs text-gray-500">{formatTime(item.date)}</p></div>
                            </div>
                        ...
                        <div className="pl-13 ml-1">
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <span className={`font-extrabold text-lg ${isGoal ? 'text-purple-600' : (isIncome ? 'text-sage-600' : 'text-coral-600')}`}>
                                    {isGoal ? 'Alvo: ' : (isIncome ? '+' : '-')} R$ {safeAmount.toLocaleString('pt-BR', {minimumFractionDigits: 2})}
                                </span>
                            </div>
